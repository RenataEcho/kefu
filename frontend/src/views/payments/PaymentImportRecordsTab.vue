<template>
  <div class="import-records-tab">
    <div class="import-toolbar">
      ...
      <n-button quaternary size="small" :loading="paymentStore.importListLoading" @click="refreshList">
        刷新列表
      </n-button>
    </div>
    ...
    <n-data-table
      ...
      :columns="columns"
      :data="paymentStore.importBatches"
      :loading="paymentStore.importListLoading"
      :bordered="false"
      :row-key="(r) => r.id"
      :scroll-x="1180"
      :single-line="false"
...
async function refreshList() {
  await paymentStore.loadImportBatches()
...
const columns = computed<DataTableColumns<PaymentImportBatch>>(() => [
  {
    ...
    renderExpand: (row) => {
      const loading = paymentStore.importDetailLoading[row.id] === true
...
watch(
  ...
  async (keys) => {
    for (const id of keys) {
      const row = paymentStore.importBatches.find((b) => b.id === id)
      if (!row || row.status === 'PROCESSING' || row.failCount === 0) continue
      if (detailById.value[id] !== undefined) continue
      try {
        const d = await paymentStore.loadImportBatchDetail(id)
  ...
  (v) => {
    ...
    if (v) {
      ...
      pollTimer = setInterval(() => {
        ...
        if (paymentStore.importBatches.some((b) => b.status === 'PROCESSING')) {
          void refreshList()
        }
      }, 5000)
    }
